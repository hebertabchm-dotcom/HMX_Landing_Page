...
const testimonials = [
  {
    ...
    content: 'A HMX mudou o jogo para nós. Saímos de um ROI de 2 para 7 em 3 meses. A previsibilidade que eles trouxeram permitiu dobrar nosso estoque.',
  },
  {
    name: 'Amanda Torres',
    role: 'Fundadora, TechStart',
    content: 'Eles não são apenas gestores de tráfego, são parceiros de negócio. O dashboard de BI nos dá clareza total de onde investir cada centavo.',
  },
  {
    name: 'Ricardo Gomes',
    role: 'Diretor Comercial, ImobPrime',
    content: 'O qualificação dos leads melhorou absurdamente. O time de vendas parou de perder tempo com curiosos e começou a fechar contratos.',
  }
];
      ...
      <Container>
        <Reveal width="100%">
            <SectionTitle 
                ...
                title="Parceiros de crescimento" 
                className="md:mb-16"
            />
        </Reveal>
                ...
                <Reveal key={i} delay={i * 0.1} className='h-full'>
                    <Card className="flex flex-col h-full hover:border-accent-primary/20 hover:-translate-y-2 transition-transform duration-300">
                        <div className="flex gap-1 mb-4 text-accent-primary">
                            {[...Array(5)].map((_, j) => (
                                <Star key={j} size={16} fill="currentColor" className="drop-shadow-[0_0_5px_rgba(14,165,233,0.5)]" />
                            ))}
                        </div>
